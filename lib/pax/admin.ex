defmodule Pax.Admin do
  ...
  > #### `use Pax.Admin` {: .info}
  >
  > When you `use Pax.Admin` this module will be imported to provide many convenience macros for defining the
  > layout of your Admin. It will declare `@behaviour Pax.Admin` and `@before_compile Pax.Admin`. It
  > will also define many attributes required by Pax.Admin for interoperability with your site. Finally it will define
  > several functions that can be used to generate paths and URLs to parts of your Admin.
  """
  ...
  defstruct [:admin_mod, :config, :resources, :active, :resource]
  ...
  @doc """
  This macro is used to define the Admin for your application. The `:router` option is required and should be
  the module name of your main site Router. This is required so that your Admin module can generate proper
  paths and urls for your Admin, which is used in the interface.

  ## Example

      use Pax.Admin, router: MyAppWeb.Router
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      def dashboard_path(), do: Pax.Admin.dashboard_path(__MODULE__)
      ...
      def resource_show_path(section \\ nil, resource, object_ids),
        do: Pax.Admin.resource_show_path(__MODULE__, section, resource, object_ids)

      def resource_edit_path(section \\ nil, resource, object_ids),
        do: Pax.Admin.resource_edit_path(__MODULE__, section, resource, object_ids)
      ...
      def resource_new_url(conn_or_socket_or_endpoint_or_uri, section \\ nil, resource),
        do: Pax.Admin.resource_new_url(__MODULE__, conn_or_socket_or_endpoint_or_uri, section, resource)

      def resource_show_url(conn_or_socket_or_endpoint_or_uri, section \\ nil, resource, object_ids),
        do: Pax.Admin.resource_show_url(__MODULE__, conn_or_socket_or_endpoint_or_uri, section, resource, object_ids)

      def resource_edit_url(conn_or_socket_or_endpoint_or_uri, section \\ nil, resource, object_ids),
        do: Pax.Admin.resource_edit_url(__MODULE__, conn_or_socket_or_endpoint_or_uri, section, resource, object_ids)
    end
  end
  ...
  def config_for(admin_mod, params, session, socket) do
    if Code.ensure_loaded?(admin_mod) and function_exported?(admin_mod, :config, 3) do
      ...
      # TODO: use Pax.Config for the Admin config
    ...
    error -> raise "invalid config in #{admin_mod}: #{Exception.message(error)}"
  end
  ...
  defmacro section(name, opts \\ [], do: body) when is_atom(name) and is_list(opts) do
    quote do
      Pax.Admin.__section__(__MODULE__, unquote(name), unquote(opts))
  ...
  def __section__(admin_mod, name, opts) do
    case Module.get_attribute(admin_mod, :pax_current_section) do
      nil ->
        put_section(admin_mod, name, opts)
  ...
  defp put_section(admin_mod, name, opts) do
    sections = Module.get_attribute(admin_mod, :pax_sections)
    ...
    if Map.has_key?(sections, name) do
      ...
      Module.put_attribute(admin_mod, :pax_current_section, section)
      Module.put_attribute(admin_mod, :pax_sections, Map.put(sections, name, section))
    end
  end

  defmacro resource(name, resource_mod, opts \\ []) when is_atom(name) and is_list(opts) do
    quote do
      Pax.Admin.__resource__(__MODULE__, unquote(name), unquote(resource_mod), unquote(opts))
    end
  end

  def __resource__(admin_mod, name, resource_mod, opts \\ []) do
    current_section = Module.get_attribute(admin_mod, :pax_current_section)
    ...
    if exists? do
      ...
      Module.put_attribute(admin_mod, :pax_resources, %Resource{
        name: name,
        path: to_string(name),
        label: Keyword.get_lazy(opts, :label, fn -> Pax.Util.Introspection.resource_name_to_label(name) end),
  ...
  #   │
  # 1 │ defmodule SandboxWeb.Admin do
  #   │ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #   │
  #   └─ lib/sandbox_web/admin.ex:1: SandboxWeb.Admin.__pax__/1
  ...
  def dashboard_path(admin_mod) do
  ...
  def resource_new_path(admin_mod, section \\ nil, resource) do
    path = admin_mod.__pax__(:path)
    section_path = section_path(section)
    resource_path = resource_path(resource)
  ...
  """
  def resource_show_path(admin_mod, section \\ nil, resource, object_ids) do
    path = admin_mod.__pax__(:path)
    section_path = section_path(section)
    resource_path = resource_path(resource)
  ...
  Get the path to the show page for a resource object.
  """
  def resource_edit_path(admin_mod, section \\ nil, resource, object_ids) do
    path = admin_mod.__pax__(:path)
    section_path = section_path(section)
    resource_path = resource_path(resource)
    object_path = object_path(object_ids)
  ...
  def resource_new_url(admin_mod, conn_or_socket_or_endpoint_or_uri, section, resource) do
    path = resource_new_path(admin_mod, section, resource)
    Phoenix.VerifiedRoutes.unverified_url(conn_or_socket_or_endpoint_or_uri, path)
  end

  def resource_show_url(admin_mod, conn_or_socket_or_endpoint_or_uri, section \\ nil, resource, object_ids) do
    path = resource_show_path(admin_mod, section, resource, object_ids)
    Phoenix.VerifiedRoutes.unverified_url(conn_or_socket_or_endpoint_or_uri, path)
  end

  def resource_edit_url(admin_mod, conn_or_socket_or_endpoint_or_uri, section \\ nil, resource, object_ids) do
    path = resource_edit_path(admin_mod, section, resource, object_ids)
    Phoenix.VerifiedRoutes.unverified_url(conn_or_socket_or_endpoint_or_uri, path)
  end
end
